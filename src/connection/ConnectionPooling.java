...
public class ConnectionPooling {
    ...
    private HashMap<Integer, Statement> getStatement(){
    	...
    	while(true){
    		...
    		if(isAvailable[i] == true) {	
    			System.out.println("#" + i + " connection is leasing...");
    			isAvailable[i] = false;
    			try {	
    				HashMap<Integer, Statement> hm = new HashMap<Integer, Statement>();
    				hm.put(i, connections[i].createStatement());
					return hm;
	...
	public ConnectionPooling(){		
		try {			
				isAvailable = new boolean [10];
				connections = new Connection[10];
				
				for(int i = 0; i < 10; i ++){	
					isAvailable[i] = true;
					Class.forName("com.mysql.jdbc.Driver").newInstance();
					Connection tmp = DriverManager.getConnection("jdbc:mysql://localhost:3306/lab2yelp","root","");
					connections[i] = tmp;
				}
				connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/lab2yelp","root","");
